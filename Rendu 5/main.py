"""
On a réparti en 3 l'usage de la couche applicative.
- On a tout d'abord des ADMINS qui s'occupent de créer chaque instance de la base de donnée
(exceptée des voyageurs qui s'inscrivent eux-mêmes)
et ont la possibilité de supprimer toutes données.
- On a ensuite créé des GESTIONNAIRES qui eux s'occupent de relier les différentes instances entre elles.
Par exemple relier des taxis avec des gares, etc.
Ils n'ont aucun droit de suppression. Ils peuvent seulement relier des données ou modifier certaines données.
- Et on a ensuite créé des CLIENTS qui peuvent créer, supprimer un compte, réserver, annuler, modifier des billets.
Ils peuvent également consulter quelques statistiques sur les différentes lignes, trajets, etc.
Et peuvent aussi rechercher des trajets, horaires de trains, etc.

De plus, on a modifié la base de données en ajoutant des utilisateurs de l'application.
En effet, on a créé une table utilisateur permettant d'identifier
les utilisateurs avec leur login et leur mot de passe.
On a ensuite créé en Python des fonctions pour se créer un compte ou se login si on a déjà un compte.
...
conn = psycopg2.connect("dbname='dbnf18p096' user='nf18p096' password='sWqlK68TnMUO' host='tuxa.sme.utc'")
cur = conn.cursor()
...
def admin():
    ...
    while choix.lower() != 'q':
        ...
        while choix not in liste_choix and choix.lower() != 'q':
...
def menu_utilisateur():
    while True:
        ...
        print("Bonjour, bienvenue dans l'application !")
        print("\033[32m========== Login ==========")

DROP TABLE IF EXISTS Gare,Taxi,TransportPublic,Hotel,Gare_Hotel,Gare_TransPublic,Gare_Taxi,Ligne,TypeTrain,
    Voyage,Dessert_Hebdomadaire,DateException,Jour_Voyage,Exception_Voyage,Train,Voyageur,Billet,Dessert,Utilisateur CASCADE;
...
DROP VIEW IF EXISTS Ligne_Voyage,Voyage_Date,voyage_ids,dessert_voyage_ids,voyage_ids_intersect,voyage_ligne_depart_gares,dessert_voyage_gares,
    ...
    check_voyage_trajet,trajet_heure_depart,trajet_heure_arrivee CASCADE;
...
/* On récupère tous les billets dont dans le trajet, gare_depart est la gare de départ */
...
CREATE VIEW Trajet_voyage_depart as
SELECT DISTINCT trajet->>'voyage' as voyage
FROM Billet b;
...
CREATE VIEW Trajet_voyage_correspondant AS
SELECT DISTINCT d.voyage as voyage
FROM Trajet_voyage_depart d 
INNER JOIN Trajet_voyage_arrivee a ON d.voyage = a.voyage;
...
EXCEPT
SELECT *
FROM (SELECT DISTINCT trajet->>'voyage' as voy
      FROM Billet
    INNER JOIN Dessert d ON d.voyage = CAST(trajet->>'voyage' AS INTEGER) AND d.gare = CAST(trajet->>'gare_arrive' as INTEGER)) AS t;
...
CREATE VIEW trajet_heure_depart AS
SELECT trajet->>'heure_depart' as heure_depart
FROM Billet
INNER JOIN Dessert d ON CAST(trajet->>'voyage' AS INTEGER) = d.voyage 
AND CAST(trajet->>'gare_depart' AS INTEGER) = d.gare AND CAST(trajet->>'heure_depart' AS TIME) = d.heure_depart;
...
CREATE VIEW trajet_heure_arrivee AS
SELECT trajet->>'heure_arrive' as heure_arrive
FROM Billet
INNER JOIN Dessert d ON CAST(trajet->>'voyage' AS INTEGER) = d.voyage AND CAST(trajet->>'gare_arrive' AS INTEGER) = d.gare AND CAST(trajet->>'heure_arrive' AS TIME) = d.heure_arrive;

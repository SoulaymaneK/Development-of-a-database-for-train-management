...
def afficher_billet_voyageur(voyageur):
    sql1 = f"""SELECT b.*,g1.nom,b.trajet->>'heure_depart',g2.nom,b.trajet->>'heure_arrive',b.trajet->>'date',b.trajet->>'num_place' FROM Billet b
                       INNER JOIN Gare g1 ON CAST(b.trajet->>'heure_depart' AS INTEGER)=g1.id \
                       INNER JOIN Gare g2 ON CAST(b.trajet->>'heure_depart' AS INTEGER)= g2.id
...
def afficher_liste_train():
    ...
    if liste_train:
        for train in liste_train:
            print(
                f"{train[0]}. Type : {train[1]}, nombre de places: {train[2]}, "
                f"Classes disponibles : {'première et seconde' if train[3] else 'seulement seconde'}, "
                f"vitesse maximale : {train[4]}")
        return liste_train
    else:
...
def modifier_billet_assurance(voyageur):
    ...
    if check_billet:
        if check_billet[3]:
            assu = "FALSE"
            choix = input("Une assurance est prise pour ce billet. "
                          "Voulez vous changer (oui/non) ? ")
        else:
            assu = "TRUE"
            choix = input(
                "Vous n'avez pas décidé de prendre une assurance sur votre billet. "
                "Voulez vous changer (oui/non) ? ")

        if choix.lower() == 'oui':
            sql3 = f"""UPDATE Billet SET assurance={assu} WHERE id={billet};"""
            cur.execute(sql3)
            conn.commit()
...
def modifier_billet_place(voyageur):
    ...
    if check_billet:
        if check_billet[3]:
            assu = "FALSE"
            choix = input("Une assurance est pris pour ce billet. "
                          "Voulez vous changer (oui/non) ? ")
        else:
            assu = "TRUE"
            choix = input(
